...
Density depends on the size of the bins that you use to group your data. In a density plot, the area of all of the bins will always add to 1. This means that if the width of the bin is less than 1, the value of the bar in the histogram can be greater than 1, even though it's not possible for something to have a greater than 100% chance of happening. When the width of the bins are exactly 1, the density is the same as the probability.

The density plotted also depends on the location of the bins, in addition to their width. 

If we simulate two sets of data using two different distributions we can see how the effect 
...
binsB = np.arange(-5, 5) #set up bins for histogram (Offset)


fig5, ax5 = plt.subplots(nrows=2, ncols=2, sharex=True, sharey=True)

ax5[0,0].hist(norm1, bins=binsA, density=True, alpha=0.7)
ax5[0,0].hist(norm4, bins=binsA, density=True, alpha=0.7)
ax5[0,0].annotate('A.', xy=(-5,0.75))

ax5[0,1].hist(norm1, bins=binsB, density=True, alpha=0.7)
ax5[0,1].hist(norm4, bins=binsB, density=True, alpha=0.7)
ax5[0,1].annotate('B.', xy=(-5,0.75))

ax5[1,0].annotate('C.', xy=(-5,0.75))

ax5[1,1].hist(norm1, bins=40, density=True, alpha=0.7)
ax5[1,1].hist(norm4, bins=40, density=True, alpha=0.7)
ax5[1,1].annotate('D.', xy=(-5,0.75))

# ax5[0,1].scatter(df.A, df.Ay, s=8) #First row, second column

# ax5[1,0].scatter(df.B, df.Bx, s=8) #Second row, first column
# ax5[1,0].set_ylabel('Random Data B', fontdict=pt12)
# ax4[1,0].set_xlabel('Random Data X', fontdict=pt12)

# ax5[1,1].scatter(df.B, df.By, s=8) #second row, second column
# ax5[1,1].set_xlabel('Random Data Y', fontdict=pt12)


fig5.suptitle('Figure 5. Displaying some randomly generated data', fontsize=16)
