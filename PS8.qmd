...
Where y is the variable we are interested in predicting and x is a variable we will use to predict it. In our diabetes data, $y$ is our measure of disease progression (`target`) and $x$ could be any one of our other variables. For this example we will use BMI as our $x$ variable. 

To find the line that best fits our data, we want to minimize the difference between the $y$ values of our line and the $y$ values of our data. This process is called linear regression.
...
While the correlation coefficent tells you how closely to variables move together, it does not tell you 
...
To determine the effect size we need ___. 
...
For this problem set please use the NBA Player dataset provided. It can be accessed through data.world, and the original source of the data is [hoopsstats.com](http://www.hoopsstats.com/basketball/) where you can find the documentation for the variables.
...
2. Prediction vs. Inference
   a. What is a question of prediction we could ask about fuel efficiency using the `NBA Player` data set.
   b. What is an inferential question we could ask about fuel efficiency using the `NBA Player` data set. 
3. Visualization: create a figure with scatterplots showing how total points scored changes with several of the numeric variables in the dataset.
4.  Calculating Correlation
   a. Based on the plot you created in Question 3, predict whether each variable (besides fuel efficiency) correlates with fuel efficiency (positive, negative, or close to 0).
   b. Calculate the correlation coefficent for each variable.
   c. If you used the `corr` method to calculate your correlation coefficent, find a different function that also calculates Pearson's correlation coefficient. What are the differences between the two functions?
