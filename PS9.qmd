...
Like Problem Set 8, this problem set also covers material discussed in [Correlation Does Not Equal Causation](https://www.youtube.com/watch?v=GtV-VYdNt_g&list=PL8dPuuaLjXtNM_Y-bUAhblSAdWRnmBUcr&index=9&t=1s). You will also need your results from PS8.
...
By looking at how all the variables correlate with `target`, we can see that BMI, blood pressure, and blood triglicerides are positively correlated with our target variable. This is in line with what we saw in our scatterplots earlier. "Good Cholesteral" (HDL) is also negatively correlated with disease progression, but the strength of the correlation is weaker. Overall, BMI seems like a pretty good choice for a predictor variable. However, if we wanted to try another variable to our model, blood triglyceride level would be a good alternative.

## Correlation vs Effect Size

While correlation tells us how closely our independent variable tracks our dependent variable, it doesn't tell us how much of a change in our dependent variable a change in our independent variable will produce. For example, if adding expensive plant food to your lemon tree produces only 5 more lemons a year, you probably wouldn't spend the money. However, if the product doubled your lemon harvest it would be worth the extra cost. Both correlation and effect size are important in evaluating our model, as the figure below illustrates. 
```{python }

n_rand = 500

df = pd.DataFrame({'X':rand.uniform(-5, 5, n_rand)})
df['Y1'] = df.X*1 + rand.normal(0, 2, n_rand) 
df['Y2'] = df.X*1 + rand.normal(0, 10, n_rand)
df['Y3'] = df.X*5 + rand.normal(0, 2, n_rand)
df['Y4'] = df.X*5 + rand.normal(0, 10, n_rand)
...
# Correlation $\neq$ Causation
In the previous problem set, we discussed predicting a dependent variable using an independent variable, and in this problem set we discussed how correlation measures the strength of the relationship between to variable. However, just because the independent and dependent variables in your model are correlated, does not mean the change in the independent variable necessarily caused the change the dependent variable. This can happen in three primary ways: reverse causation, confounding factors, and spurious correlation. 

![[Correlation - xkcd 552](https://xkcd.com/552/)](data/plots/correlation_2x.png)
...
Of the three problems, spurious correlation is the most difficult to deal with. We will discuss ways to mitigate the risk of spurious correlation in future problem sets.


# Questions

For this problem set please use the NBA Player dataset from PS8. 

1. Calculate the Pearson's $r$ for every pair of variables in your dataset.
   a. Why does the left diagonal of your matrix have the value of 1? Would those variable pairs be useful to use in your model?
   b. Which pair of variables has the strongest and weakest correlation?
   
2. Compare the correlation coefficients ($r$) for every independent variable paired with your dependent variable.
   a. Rank the independent variables in order of absolute value of correlation. Which has the strongest correlation?
   b. Using your plots from PS8, rank the independent variables based on effect size.
   c. Does the strength of the correlation depend on the units of the variables? How about the effect size?
   d. If you answered yes to either part of 2C, how could you solve that issue?
3. Are there any independent variables that are strongly positively or negatively correlated? Why might that be?
4. Are there any variables that are strongly correlated with many other variables in the dataset? Why do you think that is?
5. Which pairs of variables have a clear causal direction (ex changes in A clearly cause changes B)? Which pairs have an ambiguous causal direction?
6. If you found a strong correlation between the rank of a player (Rk) and their total points scored, would this be useful to you when trying to understand what makes a good basketball player?